...
use compositor_core::color::{ColorManagement, ColorProcessor, ColorSpaceInfo};
...
impl OcioColorManagement {
    pub fn from_file(path: &str) -> Result<Self, CompositorError> {
        let c_path = CString::new(path)
            .map_err(|_| CompositorError::Other("Invalid path string".to_string()))?;
    ...
    ) -> Result<OcioCpuProcessor, CompositorError> {
        let mut cache = self
            .processor_cache
            .lock()
            .map_err(|_| CompositorError::Other("Processor cache mutex poisoned".to_string()))?;
