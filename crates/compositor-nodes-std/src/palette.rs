...
impl ColorPaletteNode {
    ...
    pub fn load_palette_data(&self, bytes: &[u8]) -> Result<Vec<[f64; 4]>, CompositorError> {
        {
            let mut guard = self
                .loaded_file_data
                .lock()
                .map_err(|_| CompositorError::Other("Palette mutex poisoned".to_string()))?;
...
fn parse_gpl(text: &str) -> Result<Vec<[f64; 4]>, CompositorError> {
