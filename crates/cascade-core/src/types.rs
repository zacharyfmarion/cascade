...
impl RectI {
    ...
    pub fn area_checked(&self) -> Result<usize, crate::error::CascadeError> {
        let w = self.width().max(0) as u64;
        let h = self.height().max(0) as u64;
        let pixels = w
            .checked_mul(h)
            .ok_or_else(|| crate::error::CascadeError::Other("Data window too large".into()))?;
        usize::try_from(pixels)
            .map_err(|_| crate::error::CascadeError::Other("Data window too large".into()))
    }
...
/// Returns `Err(CascadeError::ImageTooLarge)` if either dimension exceeds
/// `MAX_IMAGE_DIM`, or if the total pixel count would overflow `usize`.
fn validate_dimensions(width: u32, height: u32) -> Result<(), CascadeError> {
...
impl Image {
    ...
    pub fn from_f32_data(width: u32, height: u32, data: Vec<f32>) -> Result<Self, CascadeError> {
        ...
        if data.len() != expected {
            return Err(CascadeError::InvalidImageData {
                expected,
                got: data.len(),
            });
        }
    ...
    pub fn from_f16_data(width: u32, height: u32, data: Vec<f16>) -> Result<Self, CascadeError> {
...
impl Field {
    ...
    pub fn rasterize(&self, width: u32, height: u32) -> Result<Image, CascadeError> {
...
mod tests {
    ...
    fn test_oversized_width_rejected() {
        ...
        assert!(matches!(result, Err(CascadeError::ImageTooLarge { .. })));
    }
    ...
    fn test_image_too_large_error_contains_dimensions() {
        ...
        match result {
            Err(CascadeError::ImageTooLarge { width, height, max }) => {
