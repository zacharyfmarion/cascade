    ...
    doc.get("compositor")
        ...
        .ok_or_else(|| {
            MigrationError::InvalidStructure(
                "Missing or invalid compositor.format_version field".to_string(),
            )
        })
}
...
fn set_version(doc: &mut Value, version: &str) -> Result<(), MigrationError> {
    if let Some(comp) = doc.get_mut("compositor") {
        if let Some(comp_obj) = comp.as_object_mut() {
            ...
            Err(MigrationError::InvalidStructure(
                "compositor field is not an object".to_string(),
            ))
        }
    } else {
        Err(MigrationError::InvalidStructure(
            "Missing compositor field".to_string(),
        ))
    }
}
...
mod tests {
    ...
    #[test]
    fn test_extract_version_missing_compositor() {
        let doc = json!({});
        assert!(matches!(
            extract_version(&doc),
