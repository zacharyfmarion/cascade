# Implementation Plan — Cascade Feature Gaps

## Executive Summary

This plan addresses the gaps identified in the Cascade review, ordered by priority and impact. Work is divided into 4 parallel batches that can be implemented simultaneously since they touch different files.
...
- **File:** `crates/cascade-nodes-std/src/lib.rs` (or new `utils.rs`)
...
- **File:** `crates/cascade-core/src/node.rs`
...
### 2A. Despill Node
- **File:** `crates/cascade-nodes-std/src/matte.rs` (add to existing)
- **Category:** Matte
...
### 2B. Shape / Roto Node  
- **File:** `crates/cascade-nodes-std/src/generate.rs` (add to existing)
- **Category:** Generator
...
### 2C. SaveImage Node
- **File:** new `crates/cascade-nodes-std/src/output.rs` (extend existing)
- **Category:** Output  
...
- **File:** `crates/cascade-nodes-std/src/color_ops.rs` (add to existing)
- **Category:** Color
...
### 2F. Vignette Node
- **File:** `crates/cascade-nodes-std/src/filter_ops.rs` (add to existing)
- **Category:** Filter
...
- **File:** `crates/cascade-nodes-std/src/blend.rs`
...
- **File:** `crates/cascade-nodes-std/src/matte.rs`
...
- **File:** new addition to `crates/cascade-nodes-std/src/transform.rs`
- **Category:** Transform
...
All new nodes must be registered in `crates/cascade-nodes-std/src/lib.rs`:
